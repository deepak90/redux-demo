...
<Notes>
  <ul>
    ...
    <li>definitely a clickbait title - there's a lot to talk about redux that that would not fit 55 minutes </li>
    ...
    <li>There's some amazing documentation online about the API - at the end of the day all I hope to achieve with this is if you're looking through code and you see something called reducers or actions - you know on a high level 
...
<Notes>
  <ul>
    ...
    <li>they've got cool things like shadow dom to make fast diffs - fast rendering times</li>
    <li>There's a lot of things these frameworks do, but for the sake of our topic today, let's zero in on one </li>
  </ul>
</Notes>
...
<Notes>
  <ul>
    ...
    <li>there can be multiple component and State can be set on each of these</li>
    <li>As applications grow bigger, managing states shared across components becomes a chore - what if I wanted to share the count with another component? </li>
...
<Notes>
  <ul>
    ...
    <li>real gotcha - setState is asynchronous - so, do not rely on this.state immediately after calling setState, use updater functions instead </li>
  </ul>
</Notes>
...
<Notes>
  <ul>
    ...
    <li>there are multiple stores in the application, also there seems to be something called the dispatcher </li>
    <li>Stores let the dispatcher know that they want to be notified whenever an action comes in. Then the controller views ask the stores for the latest state</li>
    <li>When the stores give the state to the controller views, they pass that state along to their child views to render</li>
    <li>The view tells the action creator to prepare an action</li>
    <li>>A Lot of moving parts here - I wanted to mention flux because it inspired better state management libraries </li>
  </ul>
</Notes>
...
<Notes>
  <ul>
    ...
    <li>Features a centralized application state - single source of truth</li>
    <li>An important point to note is that redux does not depend on react - works with any UI layer </li>
    <li>Let's look at the three principles of Redux - and also some of the core components of Redux </li>
  </ul>
</Notes>
...
<Notes>
  <ul>
    <li>The state of your application is going to be represented as a single JavaScript object.</li>
    <li>A single state tree makes it easier to debug or inspect an application</li>
    <li>This makes it easy to create universal apps, as the state from your server can be serialized and hydrated into the client with no extra coding effort</li>
    <li>it also enables you to persist your app's state in development, for a faster development cycle </li>
    <li>In Redux, the entire state of your application is kept in a store</li>
  </ul>
</Notes>
...
<Notes>
  <ul>
    ...
    <li>Rather than writing action type strings in every component that needs to create the same type of action, put the logic for creating that action in one place</li>
    <li>The parameters of the function act as a guide for what data is needed to go into the action.</li>
    <li>Testability</li>
    <li>Creating an action expresses an intent to modify the state</li>
  </ul>
</Notes>
...
<CodeSurfer
  ...
  showNumbers
  steps={[
    { range:  [1, 8], notes: "Pure functions" },
...
<Notes>
  <ul>
    ...
    <li>there is only one way to know if two JavaScript objects have the same properties. To deep-compare them. But this becomes extremely expensive in real-world apps, because of the typically large objects and the number of times they need to be compared.So one work around is to have a policy to ask developers to create a new object whenever there is a change, then send it to the framework. And if there are no changes, then send back the old object as it is. In other words, new objects represent new states.</li>
  </ul>
</Notes>
